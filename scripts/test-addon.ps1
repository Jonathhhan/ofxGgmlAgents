...
if ([string]::IsNullOrWhiteSpace($BuildDir)) {
	$BuildDir = Join-Path ([System.IO.Path]::GetTempPath()) "ofxGgmlAgents-tests"
}
...
if (Test-WindowsHost) {
	...
	Write-Step "Configuring and running ofxGgmlAgents tests with Visual Studio tools"
	Invoke-CheckedCmd "ofxGgmlAgents tests" $command
} else {
	Write-Step "Configuring ofxGgmlAgents tests"
	Invoke-CheckedNative "cmake configure ofxGgmlAgents tests" {
		cmake -S $testsDir -B $BuildDir -DCMAKE_BUILD_TYPE=$Configuration
	}
	Write-Step "Building ofxGgmlAgents tests"
	Invoke-CheckedNative "cmake build ofxGgmlAgents tests" {
		cmake --build $BuildDir --config $Configuration
	}
	Write-Step "Running ofxGgmlAgents tests"
	Invoke-CheckedNative "ctest ofxGgmlAgents tests" {
