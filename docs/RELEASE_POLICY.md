...
`ofxGgmlAgents` releases are tagged independently from `ofxGgmlCore` and the
...
Planner loops, tool registries, local model runners, memory handoff, RAG
integration, and assistant workflows should be explicit about their local
runtime, persistence, and model requirements when they land. Skeleton releases
must say clearly when no model-backed agent runtime is included.

Generated models, memory/index files, media outputs, and build outputs stay out
of git.
...
5. Confirm no generated artifacts, model binaries, memory files, or indexes are staged.
